(function ($) {
	...
	$(window).scroll(function () {
		...
		if (scroll >= 50) {
			$(".account-head-color").addClass("account-head-color-fixed");
		} else {
			$(".account-head-color").removeClass("account-head-color-fixed");
		}
	});
...
function previewImages(event) {
	...
	selectedFiles.forEach((file, index) => {
		...
		reader.onload = function (e) {
			...
			removeBtn.onclick = (ev) => {
				ev.stopPropagation(); // prevent any unwanted clicks
				removeImage(index);
			};
	...
	// Update input files after preview is set
	updateFileList();
}
...
document.addEventListener('click', function (e) {
	if (e.target.classList.contains('remove-img-btn')) {
		...
		const imgSrc = e.target.getAttribute('data-img'); // For old images
		const fileIndex = e.target.getAttribute('data-index'); // For new images
		...
		setTimeout(() => {
			...
			if (imgSrc) {
				// Old image: update hidden input
				const imagesToKeepInput = document.getElementById('imagesToKeep');
				let imagesToKeep = [];

				try {
					imagesToKeep = JSON.parse(imagesToKeepInput.value || '[]');
				} catch {
					imagesToKeep = [];
				}

				imagesToKeep = imagesToKeep.filter(src => src !== imgSrc);
				imagesToKeepInput.value = JSON.stringify(imagesToKeep);
			} else if (fileIndex !== null) {
				// New image: update selectedFiles and input
...
document.addEventListener('DOMContentLoaded', function () {
	...
	form.addEventListener('submit', function (event) {
		...
		for (let i = 0; i < files.length; i++) {
			if (files[i].size > 400 * 1024) { // 400 KB
				oversized = true;
				break;
			}
		}
		...
		if (oversized) {
			...
			alert("One or more images are larger than 400 KB. Please upload smaller images.");
		}
	});
});